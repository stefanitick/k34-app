...
import { Users, Calendar, Star, ChevronRight, Check, X, Eye } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function loadData() {
    ...
    const pendingItems: PendingItem[] = (pendingMem ?? []).map(m => ({
      id: m.id,
    ...
    const { data: sessions } = await supabase.from('sessions').select('*').gte('session_date', today).order('session_date', { ascending: true }).limit(5)
  ...
  async function viewProof(path: string) {
    ...
    if (!url) { toast.error('Could not load proof image'); return }
    setProofModal(url)
  }
          ...
          {pending.length > 0 && (
            <>
              ...
              <div className="mx-5 mb-3.5 bg-dark2 border border-white/7 rounded-[14px] overflow-hidden">
                {pending.map((item) => (
                  <div key={item.id} className="px-4 py-3 border-b border-white/7 last:border-0">
                    ...
                    <div className="flex gap-1.5">
                      {item.proof_url && (
                        <button
                          onClick={() => viewProof(item.proof_url!)}
                          disabled={proofLoading}
                          className="flex items-center gap-1 text-[11px] text-gray2 bg-dark3 border border-white/7 px-2.5 py-1.5 rounded-lg"
                        >
                          <Eye size={11} /> {proofLoading ? '...' : 'View Proof'}
                        </button>
                      )}
                      {!item.proof_url && (
                        <span className="text-[10px] text-gray italic px-2 py-1.5">No proof uploaded</span>
                      )}
                      <div className="flex gap-1.5 ml-auto">
                        <button onClick={() => approveMembership(item.id)}
                          className="bg-success/12 text-success border border-success/25 text-[11px] px-3 py-1.5 rounded-lg flex items-center gap-1">
                          <Check size={12} /> Approve
                        </button>
                        <button onClick={() => rejectMembership(item.id)}
                          className="bg-dark3 text-gray2 border border-white/7 text-[11px] px-3 py-1.5 rounded-lg flex items-center gap-1">
                          <X size={12} /> Reject
                        </button>
                      </div>
                    </div>
          ...
          <div className="px-5 mt-3.5 mb-2.5">
            ...
            <div className="bg-dark2 border border-white/7 rounded-[14px] overflow-hidden">
              {upcomingSessions.map(s => (
                <button key={s.id} onClick={() => router.push(`/admin/sessions`)}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[11px] text-gray2">{(s as Session & { attendance_count?: number }).attendance_count ?? 0}/{s.max_attendance} registered</div>
                  </div>
                  <ChevronRight size={14} className="text-gray2" />
                </button>
              ))}
